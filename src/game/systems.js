...
export function updateGame(deltaTime, { input, camera, physics, time, rng, canvas }) {
  ...
  if (Math.floor(Date.now() / 1000) % 2 === 0) {
    console.log('Game running - Player:', { x: player.x, y: player.y, onGround: player.onGround });
  }
...
function updatePlayerMovement(deltaTime, input, physics, player, updatePlayer, setPlayerVelocity) {
  ...
  if (isJumpJustPressed) {
    if (onGround || input.hasCoyoteTime()) {
      ...
      console.log('JUMPING! vy set to:', vy, 'onGround was:', onGround);
    } else {
      console.log('Jump blocked - not on ground. onGround:', onGround, 'coyoteTime:', input.hasCoyoteTime());
    }
  }
  ...
  if (y >= groundY) {
    ...
    if (!wasOnGround) {
      ...
      console.log('LANDED! y:', y, 'groundY:', groundY);
    }
  } else {
    onGround = false;
  }
  ...
  if (isJumpJustPressed) {
    console.log('Jump pressed! onGround:', onGround, 'y:', y, 'groundY:', groundY);
  }
